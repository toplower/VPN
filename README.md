...
## Конфигурация
Скопируйте файл `config.example.py` в `config.py` и задайте ваш токен бота и список ID администраторов.
Альтернативно можно указать переменные окружения `API_TOKEN` и `ADMIN_ID` (через запятую для нескольких значений).
